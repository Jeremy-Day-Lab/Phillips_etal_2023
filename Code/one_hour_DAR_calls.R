...
set.seed(1234)
setwd(dir = "/Users/rphill3/1_hour_ATAC_local/")
...
KCl_Veh_Str_1 <- dba(sampleSheet = "1_hour_ATAC.csv",
                     minOverlap = 0) #NArrow
...
KCl_Veh_Str_1 <- dba.count(KCl_Veh_Str_1,
                           bParallel = TRUE,
                           summits = 250,
...
Counts <- as.data.frame(dba.peakset(KCl_Veh_Str_1, bRetrieve=TRUE))
write.table(x         = Counts,
            file      = "1_Hour_ATAC_Counts.txt",
            quote     = FALSE,
            col.names = TRUE,
            row.names = FALSE,
            sep       = "\t")
...
KCl_Veh_Str_1 <- dba.contrast(KCl_Veh_Str_1,contrast = c("Treatment","KCl","Veh"))
# Computing results names...

#Now run the DESeq2 analysis 
KCl_Veh_Str_1 <- dba.analyze(KCl_Veh_Str_1,method = DBA_DESEQ2)
...
#Write out the DARs
OneHour_DARs <- as.data.frame(dba.report(KCl_Veh_Str_1, th = 1,bNormalized = FALSE))
#From the dba.report help page. Start:
# When bNormalized=FALSE, raw count data is used as the basis for reporting log concentration values, 
# and Fold changes are reported based on subtracting the log concentration of one sample group from the other.
# Confidence statistics (p-value/FDR) are always reported as computed by the underlying analysis package,
#which incorporate normalization factors :End. 
#Using bNormalized = FALSE to compute fold changes without shrinkage. 

#Write out the dataframe with stats for all DARs
write.table(x         = OneHour_DARs,
            file      = "One_Hour_DARs_Rn7_all.txt",
            sep       = "\t",
            quote     = FALSE,
            row.names = FALSE,
            col.names = TRUE)
...
#No significant DARs
nrow(subset(OneHour_DARs,subset=(FDR <= 0.05)))
# [1] 0


#Ggplot Volcano plot 
p1 <- ggplot(data = OneHour_DARs,aes(x = Fold,y = -log10(FDR))) + 
  geom_point(colour = "grey") +
  geom_point(data = subset(OneHour_DARs,subset=(Fold > 0 & FDR <= 0.05)),colour = "red") +
  geom_point(data = subset(OneHour_DARs,subset=(Fold < 0 & FDR <= 0.05)),colour = "dodgerblue") +
  xlim(c(-2.5,2.5)) +
  theme_bw() +
  NoGrid() +
  ylim(c(0,8)) +
  ...
  labs(x = "log2(KCl)-log2(Veh)",
       title = paste("One Hour 10mM KCl\n",nrow(subset(OneHour_DARs,subset=(FDR <= 0.05))),"DARs")) +
  annotate(geom = "text",x = -2,y = 8,label = nrow(subset(OneHour_DARs,subset=(FDR <= 0.05 & Fold > 0))),color = "red") +
  annotate(geom = "text",x = -2,y = 7.7,label = nrow(subset(OneHour_DARs,subset=(FDR <= 0.05 & Fold < 0))),color = "dodgerblue") +
  theme(plot.title = element_text(hjust = 0.5))

ggsave(filename = "One_hour_KCl_ATAC_Volcanoplot_bNormalizedFALSE.pdf",plot = p1,height = 8,width = 8)
